...
#define ASPECT_TYPE     CRYNETWORK_CONCAT(CRYNETWORK_CONCAT(std::uint, NUM_ASPECTS),_t)
typedef ASPECT_TYPE NetworkAspectType;
typedef std::uint8_t NetworkAspectID;
#define NET_ASPECT_ALL (NetworkAspectType(0xFFFFFFFF))

typedef uint32_t CryLobbyTaskID;
typedef uint32_t ECryLobbyError;
typedef uint32_t EntityId;


typedef std::uint32_t ChannelId;                                   // Network channel id (derived from GridMember)
...
struct SMessageQueueStats
{
    ...
    uint32_t m_usedPacketSize;
    uint16_t m_sentMessages;
    uint16_t m_unsentMessages;
};
...
struct SNetChannelStats
{
    ...
    uint32_t m_ping;
    uint32_t m_pingSmoothed;
    float m_bandwidthInbound;
    float m_bandwidthOutbound;
    uint32_t m_bandwidthShares;
    uint32_t m_desiredPacketRate;
    float m_currentPacketRate;
    float m_packetLossRate;
    uint32_t m_maxPacketSize;
    uint32_t m_idealPacketSize;
    uint32_t m_sparePacketSize;
    bool m_idle;
    bool m_inUse;
};
...
struct SBandwidthStatsSubset
{
    ...
    uint64_t  m_totalBandwidthSent;
    uint64_t  m_lobbyBandwidthSent;
    uint64_t  m_fragmentBandwidthSent;
    uint64_t  m_aspectPayloadBitsSent;
    uint64_t  m_rmiPayloadBitsSent;
    uint64_t  m_totalBandwidthRecvd;
    int         m_totalPacketsSent;
    uint64_t      m_totalPacketsDropped;
...
public:
    ...
    virtual void Invoke(IGameObjectExtension* extension, void* params) = 0;
...
public:
    ...
    virtual uint32_t GetReliability() const = 0;
    virtual uint32_t GetWhere() const = 0;
    //virtual void SerializeParams(TSerialize ser) = 0;
    SUBFUNC();
    virtual void Invoke(EntityId entityId, uint8_t actorExtensionId) = 0;
    virtual const char* GetDebugName() const = 0;

    void SetUniqueId(uint32_t uniqueId) { m_uniqueId = uniqueId; }
...
struct IGameContext
{
    ...
    virtual void SetGameStarted(bool started) = 0;

    virtual void PlayerIdSet(EntityId id) = 0;

    virtual const SGameStartParams& GetAppliedParams() const = 0;

    virtual void SetContextFlags(unsigned flags) = 0;
...
public:
    ...
    SUBFUNC();
    virtual void SetGameContext(IGameContext* gameContext) = 0;
    ...
    virtual void InvokeRMI(IGameObject* gameObject, IRMIRep& rep, void* params, ChannelId targetChannelFilter, uint32_t where) = 0;
    virtual void InvokeActorRMI(EntityId entityId, uint8_t actorExtensionId, ChannelId targetChannelFilter, IActorRMIRep& rep) = 0;
