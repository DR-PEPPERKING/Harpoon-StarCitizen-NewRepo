...
struct IConsole
{
    ...
    // Return:
    //   pointer to the interface ICVar
    virtual ICVar* RegisterInt64(const char* sName, __int64 iValue, int nFlags, const char* help = "", ConsoleVarFunc pChangeFunc = 0) = 0;
    ...
    virtual std::uint64_t GetCheatVarHash() = 0;
    virtual void PrintCheatVars(bool bUseLastHashRange) = 0;
    virtual char* GetCheatVarAt(std::uint32_t nOffset) = 0;
...
struct ICVar
{
    ...
    SUBFUNC();
    SUBFUNC();
    virtual const char* GetConvarName() = 0;
    SUBFUNC();
    SUBFUNC();
    SUBFUNC();
    SUBFUNC();
